...
下圖中(10,0.75,紅)，表示有一瓶酒，他的酒精濃度為10，1000棵樹裡面有750棵說他是好的，但它實際上是壞的（紅色）。
...
LIME 想要解決的問題：能不能夠找到一個容易解釋的模型 g 解釋為什麼一個個體會被分類到f預測的類別。

    概念：我們希望利用特徵去預測一個個體，可以得到一個複雜但預測準確的模型f，但是我們很難用一個簡單的模型且可解釋的模型逼近，因此我們需要利用「局部」；在個體的附近找出一個線性（簡單）的決策區域作為可解釋該個體的模型，這個決策模型在該個體附近的預測準確度與f相當，但在離該個體較遠的區域預測準確度就會大幅下降，這就是所謂的局部性。
...
- Loss function: L(f,g,$\pi_{{x}_i}$) + $\Omega(g)$

    f為複雜模型、g為可解釋模型（線性回歸），$\Omega(g)$為懲罰項希望g能簡單一點，越相似的個體，應該要給予可解釋模型與複雜模型間的誤差較重權重，因此會有一個計算「與${x}_i$相似與否」的核函數 $\pi_{{x}_i}$(kernel function)。

- 擾動樣本：進行有意義的擾動（改變${x}_i$的幾個特徵值），產生新的輸入資料（${z}_i$這些新資料是局部性的）帶入你的模型來觀察輸出結果。

-  min loss function : $\sum\pi_{{x}_i}(f(z)-g(z))^2+ \Omega(g)$

- 利用g去跑原始資料得出變數的係數觀察之間的重要程度
...
假設森林只有一棵樹，在同一棵主樹裡，某變數的minimum depth出現在第一層和第二層，minimal depth mean(5-2處)是1.5
